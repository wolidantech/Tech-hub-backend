...
--
-- Why drafts first? So admins can review/edit thumbnails, prices,
-- descriptions before the storefront goes live. The RLS policy
-- courses_public_read only exposes rows where is_published = true,
-- so drafts are hidden from anon/authenticated students.
-- =====================================================================

-- Platform settings — bank details for manual transfer page
...
-- 12 courses — inserted as DRAFTS (is_published = false)
-- Students will see NOTHING until publish_courses.sql is run.
insert into public.courses (category_id, title, slug, description, price, duration, difficulty_level, is_published)
select c.id, t.title, t.slug, t.description, 5000.00, t.duration, t.difficulty::public.difficulty_level, false
from (
  values
...
-- Example curriculum for CapCut (so admin can see structure)
...
-- Verification
-- select count(*) as total_courses, count(*) filter (where is_published) as published from public.courses;
